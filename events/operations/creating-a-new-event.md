---
layout: default
title: Creating a New CodeDay Event
parent: Operations
grand_parent: Events
nav_order: 1
---
...
> **tl;dr:**
>
> - If this is a totally new city/region, [create a region config in Contentful](creating-a-region-config.md) first.
...
{: .note }
> If this is the first event in a new region, you'll need to [create a region config](creating-a-region-config.md) first.
...
This is the "webname" from the region created in Contentful. (It's the name in the URL, e.g. `http://event.codeday.org/[webname]`). Make sure you've [created a region](creating-a-region-config.md) if we've never run events here before.
